...
class NotGate extends Component {
	constructor(schematic, x, y, rot) {
		...
		this._A = this.addPin(this._x-1, this._y+2, 'l', "A");
	...
	draw(context, gridStep) {
		...
		context.lineTo(3*gridStep, 2*gridStep);
		context.lineTo(0, 0);
		context.arc(3.5*gridStep, 2*gridStep, gridStep/2, Math.PI, -Math.PI);
		context.stroke();
	...
	toXmlNode(xml, xmlSaver) {
		...
		node.setAttribute("type", "NotGate");
	...
	static fromXmlNode(node, xmlLoader, schematic) {
		...
		var obj = new NotGate(schematic, x, y, rot);
	...
	static getNew(schematic, x, y) {
		return new NotGate(schematic, x, y, 0);
	}
}

Component.add("NotGate", NotGate);
